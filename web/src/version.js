...
export function onServerRestarting(cb) {
...
export function emitServerRestarting() {
 ...
 */
export function reloadForNewVersion(targetServerVersion) {
    if (reloading)
        return;
    if (targetServerVersion) {
        let last = null;
        try {
            last = sessionStorage.getItem(RELOAD_TARGET);
        }
        catch { /* storage may be unavailable */ }
        if (last === targetServerVersion)
            return; // already tried to reach this version — don't loop
    }
    reloading = true;
    try {
        sessionStorage.setItem(RELOAD_FLAG, '1');
        if (targetServerVersion)
            sessionStorage.setItem(RELOAD_TARGET, targetServerVersion);
    }
    catch { /* storage may be unavailable */ }
    location.reload();
}
/** True if a server version is meaningfully different from the one we are running. */
export function isDifferentVersion(serverVersion) {
    if (!serverVersion)
        return false;
    if (APP_VERSION === 'dev' || serverVersion === 'dev')
        return false;
    return serverVersion !== APP_VERSION;
}
/** Inspect a server-reported version (e.g. the X-App-Version header) and reload on mismatch. */
export function observeVersion(serverVersion) {
    if (!serverVersion)
        return;
