...
const EMPTY = { brand: [], material: [], type: [], color: [] };
/**
...
export function useFacetFilters() {
    ...
    const selection = useMemo(() => {
        const sel = { brand: [], material: [], type: [], color: [] };
        for (const field of FACET_FIELDS)
            sel[field] = params.getAll(field);
        return sel;
    }, [params]);
    const writeField = useCallback((field, values) => {
        setParams(prev => {
    ...
    const toggleOption = useCallback((field, value) => {
